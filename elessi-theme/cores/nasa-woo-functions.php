...
if (!function_exists('elessi_comment_media_upload')) :
    function elessi_comment_media_upload($comment_form) {
        ...
        $label = sprintf(esc_html__('(maximum file size is %s kB, max files is %s)', 'elessi-theme'), $max_size, $max_files);
        $label2 = '<span class="upload-img" >' . $cam_img . '</span>
                    <span class="nasa-bold-800" > <span>' . esc_html__('Select an image', 'elessi-theme') . '</span>&nbsp;' . esc_html__('or drag and drop one here', 'elessi-theme') . '</span>
                    <span class="nasa-flie-policy">' . $label.'</span>';
        
        $media_html = '<p class="nasa-form-media clear-both" data-max-file="' . esc_attr($max_files) . '" data-max-size="' . esc_attr($max_size) . '"><label class="svg_image_upload" for="nasa-comment-media">' . $label2 . '</label><input type="file" name="ns_image_upload[]" id="nasa-comment-media" class="ns_image_upload" multiple="" accept=".jpg, .jpeg, .png, .bmp, .gif, .mp4, .avi, .mpeg, .mov, .webm" /><span class="nasa-form-media-show"></span></p>';
...
if (!function_exists('elessi_preprocess_review_images')) :
    function elessi_preprocess_review_images($commentdata) {
        ...
        if (isset($files['name']) && count($files['name']) > $max_files) {
            $mess = '<strong>' . esc_html__('Error: ', 'elessi-theme') . '</strong>' . sprintf(esc_html__('Maximum number of files allowed is: %s file(s)', 'elessi-theme'), $max_files);
            wp_die($mess);
        }
        ...
        if (isset($files['size']) && !empty($files['size'])) {
            foreach ($files['size'] as $k => $size) {
                ...
                if ($size > ($max_size * 1024)) {
                    $mess = '<strong>' . esc_html__('Error: ', 'elessi-theme') . '</strong>' . sprintf(esc_html__('Max size allowed: %s kB', 'elessi-theme'), $max_size);
                    wp_die($mess);
                }
            }
        }
        ...
        if (isset($files['type']) && !empty($files['type'])) {
            foreach ($files['type'] as $type) {
                ...
                if (!in_array($type_file, array("image/jpg", "image/jpeg", "image/bmp", "image/png", "image/gif", "video/mp4", "video/avi", "video/mpeg", "video/mov", "video/webm" ))) {
                    wp_die(esc_html__('Only format accepted: JPG, JPEG, BMP, PNG, GIF, MP4, AVI, MPEG, QuickTime, or WebM', 'elessi-theme'));
                }
            }
        }
